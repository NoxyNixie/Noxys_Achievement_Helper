...
script.on_event(defines.events.on_player_cursor_stack_changed, function(e)
	...
	if player.cursor_stack and player.cursor_stack.valid_for_read and dont_build[player.cursor_stack.name] then
		...
		for _,v in pairs{defines.inventory.player_quickbar, defines.inventory.player_main, defines.inventory.player_trash, defines.inventory.player_vehicle} do
			local inv = player.get_inventory(v)
			if inv.can_insert(i) then
